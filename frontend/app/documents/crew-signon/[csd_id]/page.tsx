...
export default function CrewSignOnPreviewPage() {
  const router = useRouter();
  const params = useParams();
  const { id } = params as { id: string }; // Make sure this matches your dynamic route param
  ...
  useEffect(() => {
    async function fetchPdf() {
      ...
      const res = await fetch(`${API_URL}/documents/signon/${id}/pdf`, {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded shadow p-4 mb-4">
          <div className="w-full h-[700px] border rounded overflow-hidden bg-white">
            <iframe
              src={pdfUrl}
              title="Crew Sign On PDF Preview"
              width="100%"
              height="100%"
